# horse_racing.py - With animated horse racing
...
def render_horse_racing():
    ...
    if st.button("🏁 Start Race!"):
        ...
        while len(finished) < len(results):
            ...
            with race_area.container():
                ...
                for horse_data, _ in results:
                    ...
                    st.text(f"{name:15} | {progress_bar}")
        ...
        for idx, (h, speed) in enumerate(final_results):
            st.write(f"**{idx+1}. {h['name']}** — Speed: {speed}")
    ...
    st.markdown("---")
    if st.button("⬅️ Back to Casino Lobby", key="horse_racing_back_to_lobby"):
        st.session_state.casino_game_stage = None
        st.rerun()
