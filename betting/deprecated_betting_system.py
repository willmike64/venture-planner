        ...
        if search_horse and st.button("Search History"):
            try:
                ...
                if global_history is not None and isinstance(global_history, dict) and search_horse in global_history:
                    ...
                    with col1:
                        st.metric("Total Races", record.get('total_races', 0))
                    with col2:
                        st.metric("Total Wins", record.get('total_wins', 0))
                    with col3:
                        win_pct = (record.get('total_wins', 0) / max(record.get('total_races', 1), 1)) * 100
                        st.metric("Win Rate", f"{win_pct:.1f}%")
                    with col4:
                        st.metric("Total Earnings", f"${record.get('total_earnings', 0):,}")
